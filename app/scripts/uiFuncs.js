  ...
  try {
    ...
    var genTxWithInfo = function(data) {
      var rawTx = {
        ...
        gasPrice: data.isOffline
          ? ethFuncs.sanitizeHex(data.gasprice)
      ...
      if (typeof txData.hwType != "undefined" && txData.hwType == "ledger") {
        ...
        typeof txData.hwType != "undefined" &&
        txData.hwType == "trezor"
      ) {
        uiFuncs
          .signTxTrezor(rawTx, txData)
          .then(result => {
            callback(result);
          })
          .catch(err => {
